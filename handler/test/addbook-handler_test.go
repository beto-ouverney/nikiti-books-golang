...
func TestAdd(t *testing.T) {
	...
	tests := []struct {
		...
		{
			describe:        "Should be able to add a book",
			book:            booksMock[0],
			expectedStatus:  http.StatusCreated,
			expectedMessage: []byte(nil),
			assertMessage:   "Should be able to add a book",
		},
	}
	...
	for _, test := range tests {
		t.Run(test.describe, func(t *testing.T) {
			...
			req := httptest.NewRequest(http.MethodPost, "/books", bytes.NewBuffer(data))
			...
			path2 := fmt.Sprintf("/books/%s", url.QueryEscape(test.book.Title))
			...
			assertions.Equal(test.book, actual, test.assertMessage)
		})
...
func TestAddFailTests(t *testing.T) {
	assertions := assert.New(t)
	...
	tests := []struct {
		...
		{
			describe: "Should not be able to add a book with an empty title",
			book: entity.Book{
				Title:    "",
				Author:   "Alberto Paz",
			...
			assertMessage:   "Should not be able to add a book with an empty title",
		},
		{
			describe: "Should not be able to add a book with an empty author",
			book: entity.Book{
				Title:    "Nikiti Books",
				Author:   "",
				Category: []string{"Programming", "Golang"},
			...
			assertMessage:   "Should not be able to add a book with an empty author",
		},
		{
			describe: "Should not be able to add a book with an empty synopsis",
			book: entity.Book{
				Title:    "Nikiti Books",
				Author:   "Alberto Paz",
				Category: []string{"Programming", "Golang"},
			...
			assertMessage:   "Should not be able to add a book with an empty synopsis",
		},
		{
			describe: "Should not be able to add a book with an empty category",
			book: entity.Book{
				Title:    "Nikiti Books",
				Author:   "Alberto Paz",
				Category: []string{},
			...
			assertMessage:   "Should not be able to add a book with an empty category",
		},
		{
			describe: "Should not be able to add a book with author with less than 3 characters",
			book: entity.Book{
				Title:    "Nikiti Books",
				Author:   "Al",
				Category: []string{"Programming", "Golang"},
		...
		{
			describe: "Should not be able to add a book with title with a Synopsis with less than 30 characters",
			book: entity.Book{
				Title:    "Nikiti Books",
				Author:   "Alberto Paz",
				Category: []string{"Programming", "Golang"},
